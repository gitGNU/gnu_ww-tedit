...
FF_DAT * find_open ( const char * pattern, int mode )
{
  ...
  /* if there is a trailing "." in the file
  name we need to remove it, for these operating
  systems this means no extention, but can fail
  the later matching */
  {
    char *p;
...
int find_file ( FF_DAT * pDat, struct findfilestruct * res )
{
  ...
  while ((de = readdir( pDat->dir )) != 0)
  {
    if (match_wildarg( de->d_name, pDat->pszPattern ))
    {
      ...
      /* on VC6 stat() function fails for ".." on network paths, so I decided to check
      explicitely for both of the special directories */
      ...
      if (stat( filePath, &res->st ))
      {
        ...
        continue;  /* We may have no permitions to execute stat! */
      }
#endif
      /*
